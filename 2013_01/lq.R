...
if (!exists("data.readed") || is.null(data.readed)) {
  ...
  d2013_01 <- new("LanguageQuestionnare_from2012",when=as.Date("2013-01-01"), 
                        data=read.csv("questionnaire4.csv", stringsAsFactor = FALSE))
  data.readed <- TRUE
}
...
cat(1)
x <- summary(getQuestionnaire(sq,"2013-01")@data$Age)
cat(2)
x <- x/sum(x)*100
cat(3)
...
d <- experienceChart(sq,"ExperienceInProgramming",when=c("2010-12","2011-07","2012-05","2013-01"))
# в 2011 мы смешади 
...
t <- table(q4@data$NowLanguage, q4@data$ExperienceInProgramming)
...
t<-table(q4@data$ExperienceInLanguage,q4@data$ExperienceInProgramming)
  png(file="el.png")
